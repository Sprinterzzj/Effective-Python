...
def final_report(cc_list, counter, start_time):
    ...
    if counter[HTTPStatus.not_found]:
        plural = 's' if counter[HTTPStatus.not_found] != 1 else ''
    ...
    if every_cc:
        ...
        for cc in (c.upper() for c in cc_args):
            if len(cc) == 1 and cc in A_Z:
                ...
                raise AttributeError('** Usage error: '+msg)
...
def process_args(default_concur_req):
    ...
    # 增加一个可选参数 a, ’store_true‘意味着如果我们制定了这个参数, 他就为True, 否则为False.
    ...
    # 增加一个可选参数, 接受一个整型值, 默认值是 sys.maxsize
